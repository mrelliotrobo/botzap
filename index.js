...
// =====================================
// CONFIGURAÇÕES (COM DELAYS MAIORES)
// =====================================
const TARGET_GROUP_NAME = 'os mídia de rec';   // Nome do seu grupo
const COMMAND = '!fig';                         // Comando para criar figurinha

// Configurações de segurança (MAIS LENTO = MAIS SEGURO)
const GLOBAL_DELAY_MS = 5000;        // 5 segundos entre respostas globais
const PER_USER_DELAY_MS = 15000;     // 15 segundos por usuário (evita spam)
const PROCESS_DELAY_MIN = 4000;      // 4 segundos (mínimo)
const PROCESS_DELAY_MAX = 7000;      // 7 segundos (máximo)
const MAX_QUEUE_SIZE = 50;           // Tamanho máximo da fila
...
async function createSticker(media) {
    try {
        ...
        // Redimensiona para 512x512, mantendo proporção e preenchendo
...
async function processQueue() {
    ...
    try {
        // Controle de uso por usuário (evita spam de uma única pessoa)
        const now = Date.now();
        ...
        // Delay aleatório entre 4 e 7 segundos (para parecer humano)
...
// =====================================
// EVENTO PRINCIPAL (COM message_create)
// =====================================
client.on('message_create', async (message) => {
    ...
    // Comparação flexível (ignora maiúsculas e espaços extras)
    ...
    // Evita loop: se a mensagem citada é do bot e é sticker, ignorar
